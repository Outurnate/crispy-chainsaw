...
audioStream::audioStream() :
		sys(), device(portaudio::System::instance().defaultOutputDevice()), pos(0), analysisThread(&audioStream::analysis, this)
{
...
void audioStream::loadFile(const std::string &fileURI)
{
	...
	portaudio::DirectionSpecificStreamParameters outParams(device,
			CHANNELS, portaudio::FLOAT32, false,
...
void audioStream::analysis()
{
	...
	while (true)
	{
		...
		if (std::any_of(window.begin(), window.end(), [](const std::queue<float>& i) { return i.size() < WINDOW_SIZE; }))
		...
		for (unsigned channel = 0; channel < CHANNELS; ++channel)
		{
			...
			fftwpp::rcfft1d forward(WINDOW_SIZE, windowedSample,
					transformedSample);
			forward.fft(windowedSample, transformedSample);

			for (unsigned i = 0; i < FFT_BINS; ++i)
			{
				double d = sqrt(pow(transformedSample[i].real(), 2) + pow(transformedSample[i].imag(), 2));
...
const audioStream::fftResult audioStream::analysisResult::emptyResult = { std::vector<float>(FFT_BINS, 0.0f), std::vector<float>(FFT_BINS, 0.0f) };

audioStream::analysisResult::analysisResult()
  : analyzedFFT(3, emptyResult),
	movingAverageFFT(emptyResult),
	rangedMovingAverageFFT(
			{
...
void splitFFTRanges(const std::vector<double>& fftFrequencies, const audioStream::fftResult& source, std::unordered_map<audioStream::analysisResult::spectrumRange, audioStream::fftResult>& dest)
{
	size_t subBassIndex       = std::upper_bound(fftFrequencies.begin(), fftFrequencies.end(), 60)   - fftFrequencies.begin();
	size_t bassIndex          = std::upper_bound(fftFrequencies.begin(), fftFrequencies.end(), 250)  - fftFrequencies.begin();
	size_t lowMidrangeIndex   = std::upper_bound(fftFrequencies.begin(), fftFrequencies.end(), 500)  - fftFrequencies.begin();
	size_t midrangeIndex      = std::upper_bound(fftFrequencies.begin(), fftFrequencies.end(), 2000) - fftFrequencies.begin();
	size_t upperMidrangeIndex = std::upper_bound(fftFrequencies.begin(), fftFrequencies.end(), 4000) - fftFrequencies.begin();
	size_t presenceIndex      = std::upper_bound(fftFrequencies.begin(), fftFrequencies.end(), 6000) - fftFrequencies.begin();

	for (unsigned channel = 0; channel < CHANNELS; ++channel)
		dest[audioStream::analysisResult::spectrumRange::subBass][channel]       = std::vector<float>(source[channel].begin(),                      source[channel].begin() + subBassIndex);
	for (unsigned channel = 0; channel < CHANNELS; ++channel)
		dest[audioStream::analysisResult::spectrumRange::bass][channel]          = std::vector<float>(source[channel].begin() + subBassIndex,       source[channel].begin() + bassIndex);
	for (unsigned channel = 0; channel < CHANNELS; ++channel)
		dest[audioStream::analysisResult::spectrumRange::lowMidrange][channel]   = std::vector<float>(source[channel].begin() + bassIndex,          source[channel].begin() + lowMidrangeIndex);
	for (unsigned channel = 0; channel < CHANNELS; ++channel)
		dest[audioStream::analysisResult::spectrumRange::midrange][channel]      = std::vector<float>(source[channel].begin() + lowMidrangeIndex,   source[channel].begin() + midrangeIndex);
	for (unsigned channel = 0; channel < CHANNELS; ++channel)
		dest[audioStream::analysisResult::spectrumRange::upperMidrange][channel] = std::vector<float>(source[channel].begin() + midrangeIndex,      source[channel].begin() + upperMidrangeIndex);
	for (unsigned channel = 0; channel < CHANNELS; ++channel)
		dest[audioStream::analysisResult::spectrumRange::presence][channel]      = std::vector<float>(source[channel].begin() + upperMidrangeIndex, source[channel].begin() + presenceIndex);
	for (unsigned channel = 0; channel < CHANNELS; ++channel)
		dest[audioStream::analysisResult::spectrumRange::brilliance][channel]    = std::vector<float>(source[channel].begin() + presenceIndex,      source[channel].end());
}

void audioStream::analysisResult::update(const std::vector<double>& fftFrequencies)
{
	for (unsigned channel = 0; channel < CHANNELS; ++channel)
	{
...
void audioStream::renderImGui() const
{
	...
	for (unsigned channel = 0; channel < CHANNELS; ++channel)
	{
		...
		{
			ImGui::PlotConfig conf;
			conf.values.ys = &result.rangedLatestResult.at(analysisResult::spectrumRange(i))[channel][0];
			conf.values.count = result.rangedLatestResult.at(analysisResult::spectrumRange(i))[channel].size();
			conf.scale.min = 0;
			conf.scale.max = 100;
			conf.tooltip.show = true;
			conf.tooltip.format = "x=%.2f, y=%.2f";
	...
	for (unsigned channel = 0; channel < CHANNELS; ++channel)
	{
		...
		{
			ImGui::PlotConfig conf;
			conf.values.ys = &result.rangedMovingAverageFFT.at(analysisResult::spectrumRange(i))[channel][0];
			conf.values.count = result.rangedMovingAverageFFT.at(analysisResult::spectrumRange(i))[channel].size();
			conf.scale.min = 0;
			conf.scale.max = 100;
			conf.tooltip.show = true;
			conf.tooltip.format = "x=%.2f, y=%.2f";
